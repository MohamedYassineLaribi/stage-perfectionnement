...
async function verify() {
    try {
        ...
        const user = await User.findOne({ email: 'commercial@duralux.com' }).populate('role');

        if (!user) {
            console.log('❌ Result: User "commercial@duralux.com" NOT FOUND in database.');
            return;
        }
        ...
        if (isPasswordCorrect && user.role && user.role.name === 'Commercial') {
            ...
            console.log('⚠️ RESULT: FAILED_CHECKS - Either password mismatch or role issue.');
        }
    } catch (err) {
